...
//        int count = keyList.size();
     ...
     * 设置处于游戏中的用户的对战信息
     */
    public void setUserBoardResult(String userId, String BoardResult) {
        redisTemplate.opsForHash().put(EnumRedisKey.USER_BOARD_RESULT.getKey(), userId, BoardResult);
    }

    /**
     * 移除处于游戏中的用户的对战信息
     */
    public void removeUserBoardResult(String userId) {
        redisTemplate.opsForHash().delete(EnumRedisKey.USER_BOARD_RESULT.getKey(), userId);
    }

    /**
     * 获取处于游戏中的用户的对战信息
     */
    public String getUserBoardResult(String userId) {
        Object result = redisTemplate.opsForHash().get(EnumRedisKey.USER_BOARD_RESULT.getKey(), userId);
        if (result != null){
            return redisTemplate.opsForHash().get(EnumRedisKey.USER_BOARD_RESULT.getKey(), userId).toString();
        }
        else return null;
    }
