...
// let web3 = new Web3(Web3.givenProvider || "wss://mainnet.infura.io/ws/v3/6c355a1bd0d7487e868ae499d320b4d9");
...
function get_proof_for_board_guess(initial_board, nonces, guess) {
    let merkle_tree = build_merkle(initial_board, nonces);
    // console.log("merkle tree: " + merkle_tree[merkle_tree.length - 1][0])
...
function verify_opening(opening, nonce, proof, commitment, guess) {
    ...
    console.log("curr_commit :" + curr_commit)
...
async function getRevertReason(txHash){
    ...
    // result.then((r) => {
    //     console.log(r);
    //     res = r.startsWith('0x') ? result : `0x${result}`
    // })

    // result = result.startsWith('0x') ? result : `0x${result}`

    if (res && res.substr(138)) {

        const reason = web3.utils.toAscii(res.substr(138))
